...
**Treatment Targets:**
1. ⬇️ Emotional repression
2. ⬇️ Defensive functioning
3. ⬇️ Negative affect
...
- Test: Treatment → Process → Depression
...
```{r trajectories, fig.width=12, fig.height=7}
...
```{r effect-sizes, fig.width=12, fig.height=7}
...
ggplot(effect_sizes, aes(x = Time_numeric, y = Cohens_d)) +
  geom_hline(yintercept = 0, linetype = "solid", color = "gray40", size = 1) +
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "gray50", size = 1) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper),
              alpha = 0.3, fill = "#E63946") +
  geom_line(color = "#E63946", size = 2) +
  geom_point(color = "#E63946", size = 6) +
  geom_label(aes(label = sprintf("d = %.2f", Cohens_d)),
            nudge_y = 0.3, size = 6, fontface = "bold") +
  annotate("text", x = 2.2, y = 0.8,
           label = "Large effect", size = 5) +
  scale_x_continuous(
    breaks = c(0, 1, 2),
...
.center[
## **Answer to RQ1:**
### Effects are large, durable, and increase over time ✅
]
...
```{r process-measures, fig.width=13, fig.height=7.5}
...
```{r correlations, fig.width=10, fig.height=6}
...
ggplot(cor_data, aes(x = reorder(Process, abs(Correlation)), y = Correlation)) +
  ...
  labs(
    title = "Strong Concurrent Associations Between Process and Depression Changes",
    subtitle = "Correlations between change scores (baseline to follow-up) in ISTDP group",
    x = NULL,
...
.center[
## **Answer to RQ2:**
### ISTDP produces large changes in all proposed mechanisms ✅
### Changes are strongly correlated with depression improvement ✅
]
...
Test whether:
- Process(Baseline) → Depression(Post), **OR**
- Depression(Baseline) → Process(Post)

--

.pull-left[
### **Distress**
- Process → Depression: *t* = -0.87, *p* = .39
- Depression → Process: *t* = 1.32, *p* = .19
- ❌ **No clear temporal order**

### **Emotional Repression**
- Process → Depression: *t* = 0.43, *p* = .67
- Depression → Process: *t* = -0.21, *p* = .83
- ❌ **No clear temporal order**
]
...
.pull-right[
### **Negative Affect**
- Process → Depression: *t* = -1.15, *p* = .25
- Depression → Process: *t* = 0.89, *p* = .38
- ❌ **No clear temporal order**
...
.center[
## **Answer to RQ4:**
### No evidence of temporal precedence ❌
...
### **4. Concurrent, Not Sequential** ⚠️
- No temporal precedence
- Process and depression change **together**
- Not: Process → Depression
- Instead: Treatment → (Process AND Depression)
]
...
### ❌ **BUT: Mechanisms don't mediate as expected**
...
```
heshmati-reanalysis/
├── manuscript.Rmd          # Full manuscript
├── manuscript.pdf          # Rendered PDF
├── presentation.Rmd        # This presentation!
├── data/                   # Public dataset
├── tests/                  # 75 validation tests
...
├── check-analyses.R        # Run all tests
└── docs/                   # Documentation
    ├── testing.md
...
Rscript check-analyses.R  # All tests pass ✅
```
...
1. ✅ ISTDP works (very large effects)
2. ✅ Process measures change
...
demo_summary <- baseline %>%
  summarise(
    N = n(),
    `Age (M±SD)` = sprintf("%.1f ± %.1f", mean(age, na.rm = TRUE),
                          sd(age, na.rm = TRUE)),
    `Female (%)` = sprintf("%.1f",
    ...
    `Previous Trials (M±SD)` = sprintf("%.1f ± %.1f",
...
- $M$ = Process change (Baseline → Post-treatment)
